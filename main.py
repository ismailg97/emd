...
if __name__ == '__main__':
    ...
    #ages = pd.Series(get_meta(mat_path, db), name="Age").astype(int)
    ...
    test_image_df = images.sample(1, random_state=np.random.randint(1000)).reset_index(drop=True)
    ...
    for i in train_df["Filepath"]:
        ...
        #image = np.resize(image, (244, 244, 3))
        imgs.append(image)
    imgs = np.stack(imgs)
    print(imgs.shape)
    ...
    while userInput != 4:
        match userInput:
            case "1":
                ...
                model_path = "./checkpoints/regressorMSE/regressorMSE.ckpt"
            ...
            case "2":
                checkpoint_path = "./checkpoints/classificatorXE/classificatorXE.ckpt"
                checkpoint_path_check = "./checkpoints/classificatorXE/checkpoint"
                ...
                out8 = resblock(out7, filters=512, strides=0)
                out = tf.keras.layers.GlobalAveragePooling2D()(out8)
                output = tf.keras.layers.Dense(10, activation="relu")(out)
            ...
            case "3":
                checkpoint_path = "./checkpoints/classificatorEMD/classificatorEMD.ckpt"
                checkpoint_path_check = "./checkpoints/classificatorEMD/checkpoint"
        ...
        if os.path.isfile(checkpoint_path_check):
